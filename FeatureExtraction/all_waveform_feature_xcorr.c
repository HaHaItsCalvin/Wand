 ...
 * 
 * The autocorrelation for X, Z axes are returned for each file
 * 
 * The cross correlation between the X and Z axes the two files are also
 * returned.
 * 
 * To obtain the cross correlation between X and Z axes of the same file, use
...
main(int argc, char **argv)
{
	...
	float 	       *amplitude_vector1z;
	float          *amplitude_vector2x;
	float          *amplitude_vector2y;
	float  	       *amplitude_vector2z;
	float		sample_rate;
	...
	float	       *cross_corr1x2z;
	float	       *cross_corr1z2x;	
	if (argc != 5) {
		fprintf(stderr,
			"Error - check usage\n"
			);
	...
	amplitude_vector1z = (float *)malloc(sizeof(float) * N_SAMPLES);
	...
	while ((read = getline(&line, &len, fp)) != -1) {
		/* parse the data */
		rv = sscanf(line, "%f, %f, %f, %f\n", &time_vector1[i], &amplitude_vector1x[i], &amplitude_vector1y[i],&amplitude_vector1z[i]);
		if (rv != 4) {
			fprintf(stderr,
				"%s %d \'%s\'\n",
				"Skipping line",
	...
	amplitude_vector2z = (float *)malloc(sizeof(float) * N_SAMPLES);
	/*
	...
	while ((read = getline(&line, &len, fp)) != -1) {
		/* parse the data */
		rv = sscanf(line, "%f, %f, %f, %f\n", &time_vector2[i], &amplitude_vector2x[i], &amplitude_vector2y[i], &amplitude_vector2z[i] );
		if (rv != 4) {
			fprintf(stderr,
				"%s %d \'%s\'\n",
				"Skipping line",
	...
	cross_corr1zz = (float *)malloc(sizeof(float) * 2 * vector_length);
	cross_corr2zz = (float *)malloc(sizeof(float) * 2 * vector_length);
	cross_corr1x2z = (float *)malloc(sizeof(float) * 2 * vector_length);
	cross_corr1z2x = (float *)malloc(sizeof(float) * 2 * vector_length);

	printf("Number of input samples: %i \n", vector_length);

	xcorr(amplitude_vector1x, amplitude_vector1x, cross_corr1xx, vector_length);
	xcorr(amplitude_vector1z, amplitude_vector1z, cross_corr1zz, vector_length);
	xcorr(amplitude_vector2x, amplitude_vector2x, cross_corr2xx, vector_length);
	xcorr(amplitude_vector2z, amplitude_vector2z, cross_corr2zz, vector_length);
	xcorr(amplitude_vector1x, amplitude_vector2z, cross_corr1x2z, vector_length);
	xcorr(amplitude_vector1z, amplitude_vector2x, cross_corr1z2x, vector_length);
