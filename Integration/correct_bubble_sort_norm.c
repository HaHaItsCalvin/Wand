...
void findMin(float * min1,float *min2, float *absMin)
{
	if (*min1 < *min2)
		*absMin=*min1;
	else 
...
void findMax (float *max1, float *max2, float *absMax)
{
	if (*max1 > *max2)
		*absMax=*max1;
	else
...
int main(int argc, char **argv)
{
    ...
    // file 1
		...
		continue;
			...
			time_vector[i]=time_hold;
			amplitude_vector[i]=amp_hold;
		}
                i++;
        }
        fclose(fp);
...
/*
    ...
    printf("\n");
    printf("Begin Processing 2nd File Values~~~~~~~\n");
    //print values for 2nd file
    printf("Time Vector1 Values\n");
    for (i1=0; i1 < vector_length1; i1++){
        printf("%f\n", time_vector1[i1]);
    }
    
    printf("Amplitude Vector1 Values\n");
